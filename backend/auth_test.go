...
func setupTestRouterWithConfig(t *testing.T, allowRegistration bool) *httptest.Server {
	...
	// db is the backend-agnostic seam passed to core handlers; pool stays for the
	// PostgreSQL-only SQL-query handlers and the cleanup statements below.
	db := pgxDB{pool: pool}
	...
	r.Post("/api/register", handleRegister(db, allowRegistration))
	r.Post("/api/login", handleLogin(db))
	r.Post("/api/passkey-login/begin", handlePasskeyLoginBegin(db, wan))
	r.Post("/api/passkey-login/finish", handlePasskeyLoginFinish(db, wan))
	r.Group(func(r chi.Router) {
		r.Use(requireAuth)
		r.Post("/api/logout", handleLogout(db))
		r.Get("/api/me", handleMe)
		r.Put("/api/me/email", handleChangeEmail(db))
		r.Put("/api/me/password", handleChangePassword(db))
		r.Get("/api/me/passkeys", handleListPasskeys(db))
		r.Put("/api/me/passkeys/{passkeyID}", handleUpdatePasskey(db))
		r.Delete("/api/me/passkeys/{passkeyID}", handleDeletePasskey(db))
		r.Post("/api/me/passkeys/register/begin", handlePasskeyRegisterBegin(db, wan))
		r.Post("/api/me/passkeys/register/finish", handlePasskeyRegisterFinish(db, wan))
		r.Post("/api/logs", handleCreateLog(db))
		r.Get("/api/logs", handleListLogs(db))
		r.Get("/api/logs/{logID}", handleGetLog(db))
		r.Put("/api/logs/{logID}", handleUpdateLog(db))
		r.Delete("/api/logs/{logID}", handleDeleteLog(db))
		r.Put("/api/logs/{logID}/placement", handleUpdateLogPlacement(db))
		r.Put("/api/logs/{logID}/pin", handlePinLog(db))
		r.Put("/api/logs/{logID}/home-position", handleUpdateLogHomePosition(db))
		r.Post("/api/folders", handleCreateFolder(db))
		r.Get("/api/folders", handleListFolders(db))
		r.Put("/api/folders/{folderID}", handleRenameFolder(db))
		r.Put("/api/folders/{folderID}/move", handleMoveFolder(db))
		r.Delete("/api/folders/{folderID}", handleDeleteFolder(db))
		r.Post("/api/logs/{logID}/entries", handleCreateLogEntry(db))
		r.Get("/api/logs/{logID}/entries", handleListLogEntries(db))
		r.Put("/api/logs/{logID}/entries/{entryID}", handleUpdateLogEntry(db))
		r.Delete("/api/logs/{logID}/entries/{entryID}", handleDeleteLogEntry(db))
		r.Post("/api/logs/{logID}/share-token", handleCreateShareToken(db))
		r.Delete("/api/logs/{logID}/share-token", handleDeleteShareToken(db))
		r.Get("/api/logs/{logID}/shares", handleListShares(db))
		r.Delete("/api/logs/{logID}/shares/{shareID}", handleRemoveShare(db))
		r.Get("/api/join/{token}", handleGetShareInfo(db))
		r.Post("/api/join/{token}", handleJoinLog(db))
