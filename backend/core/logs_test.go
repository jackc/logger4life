...
func TestLogActionsScopeStoreCallsToContextUser(t *testing.T) {
	store := &fakeLogStore{log: Log{ID: "log-1"}, logs: []Log{{ID: "log-1"}}}
	...
	if _, err := GetLog.Call(ctx, app, GetLogParams{LogID: "log-1"}); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := UpdateLog.Call(ctx, app, UpdateLogParams{LogID: "log-2", Name: "Pushups"}); err != nil {
		t.Fatal(err)
	}
	if store.scopeUserID != "user-1" || store.logID != "log-2" || store.name != "Pushups" {
		t.Fatalf("UpdateLog store call = (%q, %q, %q)", store.scopeUserID, store.logID, store.name)
	}

	if _, err := DeleteLog.Call(ctx, app, DeleteLogParams{LogID: "log-3"}); err != nil {
		t.Fatal(err)
	}
	if store.scopeUserID != "user-1" || store.logID != "log-3" {
		t.Fatalf("DeleteLog store call = (%q, %q)", store.scopeUserID, store.logID)
	}

	logs, err := ListLogs.Call(ctx, app, ListLogsParams{})
	if err != nil || len(logs) != 1 || logs[0].ID != "log-1" {
...
func TestLogActionsRequireAuthenticationBeforeCallingStore(t *testing.T) {
	...
	calls := []struct {
		...
		{"get_log", func() error { _, e := GetLog.Call(ctx, app, GetLogParams{LogID: "log-1"}); return e }},
		{"update_log", func() error {
			_, e := UpdateLog.Call(ctx, app, UpdateLogParams{LogID: "log-1", Name: "V"})
			return e
		}},
		{"delete_log", func() error { _, e := DeleteLog.Call(ctx, app, DeleteLogParams{LogID: "log-1"}); return e }},
...
func TestLogParamsValidationRunsBeforeStore(t *testing.T) {
	...
	for _, c := range rejected {
		...
		update := UpdateLogParams{LogID: "log-1", Name: c.params.Name, Fields: c.params.Fields}
...
func TestLogActionsPropagateStoreSentinels(t *testing.T) {
	...
	if _, err := GetLog.Call(ctx, app, GetLogParams{LogID: "log-1"}); !errors.Is(err, ErrLogNotFound) {
		t.Fatalf("get_log error = %v, want ErrLogNotFound", err)
	}
	if _, err := DeleteLog.Call(ctx, app, DeleteLogParams{LogID: "log-1"}); !errors.Is(err, ErrLogNotFound) {
