...
func (s *Store) CreateFolder(ctx context.Context, id, userID, name string, parent *string) (core.Folder, error) {
	...
	err := s.InTx(ctx, func(ctx context.Context) error {
		tx := s.conn(ctx)
		if parent != nil {
			err := folderOwned(ctx, tx, *parent, userID)
		...
		var position int
		if err := tx.QueryRow(ctx, `SELECT COALESCE(max(position)+1,0) FROM folders WHERE user_id=$1 AND parent_folder_id IS NOT DISTINCT FROM $2`, userID, parent).Scan(&position); err != nil {
			return err
		}
		var err error
		folder, err = scanFolder(tx.QueryRow(ctx, `INSERT INTO folders(id,user_id,parent_folder_id,name,position) VALUES($1,$2,$3,$4,$5) RETURNING id,name,parent_folder_id,position,created_at,updated_at`, id, userID, parent, name, position))
		return err
	})
	return folder, err
}
...
func (s *Store) MoveFolder(ctx context.Context, userID, folderID string, parent *string, pos int) error {
	return s.InTx(ctx, func(ctx context.Context) error {
		...
		var oldPos int
		e := tx.QueryRow(ctx, `SELECT user_id,parent_folder_id,position FROM folders WHERE id=$1`, folderID).Scan(&owner, &oldParent, &oldPos)
		if errors.Is(e, errNoRows) || (e == nil && owner != userID) {
...
func (s *Store) DeleteFolder(ctx context.Context, userID, folderID string) error {
	return s.InTx(ctx, func(ctx context.Context) error {
		...
		var parent *string
		var pos int
		e := tx.QueryRow(ctx, `SELECT user_id,parent_folder_id,position FROM folders WHERE id=$1`, folderID).Scan(&owner, &parent, &pos)
		if errors.Is(e, errNoRows) || (e == nil && owner != userID) {
			return core.ErrFolderNotFound
		}
		if e != nil {
