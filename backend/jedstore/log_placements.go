...
func (s *Store) UpdateLogPlacement(ctx context.Context, userID, logID string, ch domain.LogPlacementChange) error {
	return s.InTx(ctx, func(ctx context.Context) error {
		...
		var oldPos int
		e := tx.QueryRow(ctx, `SELECT folder_id,position FROM user_log_placements WHERE user_id=$1 AND log_id=$2`, userID, logID).Scan(&oldFolder, &oldPos)
...
func (s *Store) PinLog(ctx context.Context, userID, logID string, change domain.HomePinChange) error {
	return s.InTx(ctx, func(ctx context.Context) error {
		tx := s.conn(ctx)
		var current bool
		e := tx.QueryRow(ctx, `SELECT pinned_to_home FROM user_log_placements WHERE user_id=$1 AND log_id=$2`, userID, logID).Scan(&current)
		if errors.Is(e, errNoRows) {
			return core.ErrLogNotFound
		}
		if e != nil {
...
func (s *Store) UpdateLogHomePosition(ctx context.Context, userID, logID string, change domain.HomeOrderChange) error {
	...
	return s.InTx(ctx, func(ctx context.Context) error {
		...
		var pinned bool
		var old int
		e := tx.QueryRow(ctx, `SELECT pinned_to_home,home_position FROM user_log_placements WHERE user_id=$1 AND log_id=$2`, userID, logID).Scan(&pinned, &old)
		if errors.Is(e, errNoRows) {
			return core.ErrLogNotFound
		}
		if e != nil {
