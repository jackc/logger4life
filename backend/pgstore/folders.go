...
func (s *Store) CreateFolder(ctx context.Context, userID, name string, parent *string) (core.Folder, error) {
	if parent != nil {
		e := folderOwned(ctx, s.pool, *parent, userID)
	...
	return scanFolder(s.pool.QueryRow(ctx, `INSERT INTO folders(user_id,parent_folder_id,name,position) SELECT $1,$2,$3,COALESCE(max(position)+1,0) FROM folders WHERE user_id=$1 AND parent_folder_id IS NOT DISTINCT FROM $2 RETURNING id,name,parent_folder_id,position,created_at,updated_at`, userID, parent, name))
}
func (s *Store) ListFolders(ctx context.Context, userID string) ([]core.Folder, error) {
	rows, e := s.pool.Query(ctx, `SELECT id,name,parent_folder_id,position,created_at,updated_at FROM folders WHERE user_id=$1 ORDER BY parent_folder_id NULLS FIRST,position`, userID)
	if e != nil {
		return nil, e
	}
	defer rows.Close()
...
func (s *Store) RenameFolder(ctx context.Context, userID, folderID, name string) (core.Folder, error) {
	f, e := scanFolder(s.pool.QueryRow(ctx, `UPDATE folders SET name=$1,updated_at=now() WHERE id=$2 AND user_id=$3 RETURNING id,name,parent_folder_id,position,created_at,updated_at`, name, folderID, userID))
	if errors.Is(e, pgx.ErrNoRows) {
