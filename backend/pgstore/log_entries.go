...
func (s *Store) LogFieldDefinitions(ctx context.Context, userID, logID string) ([]domain.FieldDefinition, error) {
	...
	e := s.pool.QueryRow(ctx, `SELECT l.fields FROM logs l JOIN user_log_placements p ON p.log_id=l.id AND p.user_id=$1 WHERE l.id=$2`, userID, logID).Scan(&defs)
...
func (s *Store) ListLogEntries(ctx context.Context, userID, logID string) ([]domain.LogEntry, error) {
	...
	rows, e := s.pool.Query(ctx, `SELECT le.id,le.log_id,le.user_id,u.username,le.fields,le.occurred_at,le.created_at,le.updated_at FROM log_entries le JOIN users u ON u.id=le.user_id WHERE le.log_id=$1 ORDER BY le.occurred_at DESC`, logID)
	if e != nil {
		return nil, e
	}
	defer rows.Close()
...
func (s *Store) DeleteLogEntry(ctx context.Context, userID, logID, entryID string) error {
	...
	tag, e := s.pool.Exec(ctx, `DELETE FROM log_entries WHERE id=$1 AND log_id=$2`, entryID, logID)
